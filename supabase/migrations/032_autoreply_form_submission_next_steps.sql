-- =============================================================
-- StudyAlready - Migration 032 : email client automatique (suite)
-- =============================================================
-- But:
-- - Conserver l'email admin "nouvelle demande"
-- - Envoyer aussi un email automatique au client avec la suite
--   apres une soumission dans public.form_submissions.
--
-- Prerequis:
-- - migration 005 executee (private_settings + send_admin_email)
-- - extension pg_net activee
--
-- Idempotent: peut etre relance sans risque.
-- =============================================================

-- Envoi direct a une adresse (utile pour les formulaires sans user_id)
...
BEGIN
  ...
  IF api_key IS NULL OR api_key = '' OR api_key = 'REMPLACER_PAR_VOTRE_CLE_RESEND' THEN
    RAISE NOTICE 'send_email_to_address: RESEND_API_KEY non configuree, envoi ignore';
    RETURN NULL;
  END IF;
...
EXCEPTION WHEN undefined_table THEN
  RAISE NOTICE 'send_email_to_address: private_settings introuvable (migration 005 requise)';
  RETURN NULL;
END;
$$;
...
-- Etend la fonction existante de notification soumission
...
BEGIN
  label := CASE NEW.form_type
    ...
    WHEN 'prequalification' THEN 'Pre-qualification'
    WHEN 'rejoindre-reseau' THEN 'Reseau'
    ...
    WHEN 'departs-groupes' THEN 'Departs groupes'
    ELSE NEW.form_type
  END;
  ...
  PERFORM public.send_admin_email(
    ...
    '<div style="font-family:Inter,Arial,sans-serif;max-width:600px;margin:0 auto;color:#0a2540">' ||
      ...
      '<div style="background:white;border:1px solid #e2e8f0;border-top:none;border-radius:0 0 12px 12px;padding:20px 24px">' ||
        ...
        '<p style="margin:0;color:#64748b;font-size:12px">Recu le ' || to_char(NEW.created_at AT TIME ZONE 'Europe/Brussels', 'DD/MM/YYYY a HH24:MI') || '</p>' ||
      ...
      '</div>' ||
      '<p style="text-align:center;color:#94a3b8;font-size:11px;margin-top:12px">Notification automatique StudyAlready</p>' ||
    '</div>'
  );

  -- Email client (nouveau): explique la suite
  IF NEW.email IS NOT NULL AND length(trim(NEW.email)) > 3 THEN
    ...
    client_subject := 'StudyAlready - Demande bien recue, voici la suite';
    ...
    client_html :=
      '<div style="font-family:Inter,Arial,sans-serif;max-width:620px;margin:0 auto;color:#0a2540">' ||
        '<div style="background:#0a2540;color:white;padding:22px 26px;border-radius:12px 12px 0 0">' ||
          '<p style="margin:0;color:#f5b800;font-size:11px;letter-spacing:2px;text-transform:uppercase">StudyAlready</p>' ||
          '<h1 style="margin:8px 0 0 0;font-size:22px">Demande bien recuee</h1>' ||
        '</div>' ||
        '<div style="background:white;border:1px solid #e2e8f0;border-top:none;border-radius:0 0 12px 12px;padding:24px 26px">' ||
          ...
          '<p style="margin:0 0 14px 0;font-size:15px;line-height:1.6">Merci pour votre demande. Notre equipe va l''analyser et vous repondre sous 48 h ouvrees.</p>' ||
          '<div style="margin:18px 0;background:#f8fafc;border:1px solid #e2e8f0;border-radius:10px;padding:14px 16px">' ||
            '<p style="margin:0 0 8px 0;font-size:14px;font-weight:600;color:#0a2540">Prochaine etape</p>' ||
            '<p style="margin:0;font-size:14px;line-height:1.6;color:#334155">Merci de telecharger, remplir puis renvoyer ce document PDF :</p>' ||
            '<p style="margin:12px 0 0 0">' ||
              '<a href="https://www.studyalready.com/assets/docs/StudyAlready-Dossier-FWB.pdf" style="background:#f5b800;color:#0a2540;text-decoration:none;padding:10px 16px;border-radius:8px;display:inline-block;font-weight:700;font-size:13px">Telecharger le PDF StudyAlready -></a>' ||
            '</p>' ||
          '</div>' ||
          '<p style="margin:0 0 8px 0;font-size:14px;color:#334155">Une fois complete, vous pouvez nous le renvoyer :</p>' ||
          '<ul style="margin:0 0 14px 18px;padding:0;color:#334155;font-size:14px;line-height:1.6">' ||
            '<li>par email : <a href="mailto:contact@studyalready.com" style="color:#1e3a8a">contact@studyalready.com</a></li>' ||
            '<li>ou sur WhatsApp : <a href="https://wa.me/32465339448" style="color:#1e3a8a">+32 465 33 94 48</a></li>' ||
          '</ul>' ||
          '<p style="margin:0;color:#64748b;font-size:12px;line-height:1.5">Email automatique StudyAlready.</p>' ||
        '</div>' ||
      '</div>';
